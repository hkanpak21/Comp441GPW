#!/usr/bin/env python3
"""
Experiment: Paraphrase Attack on Watermarked Text

Tests watermark detection after applying paraphrase attack (Pegasus model).
Most complex attack - submit last.

Usage:
    python experiment_scripts/exp_attack_paraphrase.py --model opt-1.3b --watermark unigram --num_samples 200
"""
...
DTYPE = config.DTYPE
PARAPHRASE_ATTACK_CONFIG = config.PARAPHRASE_ATTACK_CONFIG
...
def main():
    parser = create_experiment_parser("Paraphrase Attack Experiment")
    parser.add_argument('--paraphrase_model', type=str, default="tuner007/pegasus_paraphrase",
                        help='Paraphrase model name')
    parser.add_argument('--num_beams', type=int, default=3,
                        help='Number of beams for paraphrase generation')
    args = parser.parse_args()
    
    print("=" * 80)
    print("EXPERIMENT: Paraphrase Attack on Watermarked Text")
    print("=" * 80)
    ...
    # Load attack
    print(f"\n[4/6] Loading attack: Paraphrase (Pegasus)...")
    print("  This may take a few minutes to download the paraphrase model...")
    attack_params = {
        "num_beams": args.num_beams,
        "temperature": 1.5,
        "device": device
    }
    attack = load_attack("paraphrase_pegasus", attack_params)
    print(f"✓ Attack loaded: {attack.__class__.__name__}")
    
    # Setup CSV logger
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_csv = os.path.join(args.output_dir, 
                              f"exp_attack_paraphrase_{args.model}_{args.watermark}_{timestamp}.csv")
    ...
    fieldnames = [
        ...
        'num_tokens', 'generation_time', 'attack_time', 
        'detection_time_original', 'detection_time_attacked'
    ]
    ...
    print("  NOTE: Paraphrase attack is slow, this will take a while...")
    results = []
    
    start_exp_time = time.time()
    
    for idx, prompt in enumerate(prompts):
        if (idx + 1) % 5 == 0:  # Log more frequently for slow attack
            elapsed = time.time() - start_exp_time
            avg_time = elapsed / (idx + 1)
            remaining = avg_time * (len(prompts) - idx - 1)
    ...
    metrics = {
        'experiment': 'paraphrase_attack',
        'model': args.model,
        'watermark': args.watermark,
        'paraphrase_model': args.paraphrase_model,
        'num_samples': len(results),
        'total_time': total_exp_time,
        'avg_attack_time': sum(r['attack_time'] for r in results) / len(results),
    ...
    print(f"Avg attack time: {metrics['avg_attack_time']:.2f}s per sample")
