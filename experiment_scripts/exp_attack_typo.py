...
Tests watermark detection after applying character-level typo attack.
Simple attack - submit third.

Usage:
    python experiment_scripts/exp_attack_typo.py --model opt-1.3b --watermark unigram --num_samples 200
"""
...
DTYPE = config.DTYPE
TYPO_ATTACK_CONFIG = config.TYPO_ATTACK_CONFIG
...
def main():
    parser = create_experiment_parser("Typo Attack Experiment")
    parser.add_argument('--typo_rate', type=float, default=0.05,
                        help='Character typo rate')
    args = parser.parse_args()
    
    print("=" * 80)
    print("EXPERIMENT: Typo Attack on Watermarked Text")
    print("=" * 80)
    print(f"Model: {args.model}")
    print(f"Watermark: {args.watermark}")
    print(f"Num samples: {args.num_samples}")
    print(f"Typo rate: {args.typo_rate}")
    ...
    attack_params = {"edit_rate": args.typo_rate}
    ...
    output_csv = os.path.join(args.output_dir, 
                              f"exp_attack_typo_{args.model}_{args.watermark}_{timestamp}.csv")
