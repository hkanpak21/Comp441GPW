#!/usr/bin/env python3
"""
GPW-SP Watermark Experiments - All attacks

Usage:
    python exp_watermark_gpw.py --model opt-1.3b --num_samples 200
"""
...
def main():
    ...
    print("=" * 70)
    print(f"GPW-SP WATERMARK EXPERIMENTS")
    print("=" * 70)
    ...
    output_csv = os.path.join(args.output_dir, f"results_gpw_sp_{args.model}_{timestamp}.csv")
    ...
    # Load watermarker
    print(f"\n[3/4] Loading GPW-SP watermarker...")
    wm_params = WATERMARK_PARAMS[wm_name]
    watermarker = load_watermarker("gpw", model, tokenizer, wm_params, device)  # "gpw" loader handles variants
    params_str = f"alpha={wm_params.get('alpha', 2.0)}, omega={wm_params.get('omega', 2.0)}, variant=GPW-SP"
    print(f"✓ GPW-SP loaded: {params_str}")
    ...
    for attack_name in attacks:
        ...
        for idx, (prompt, wm_text) in enumerate(zip(prompts, watermarked_texts)):
            ...
            try:
                ...
                row = {
                    'experiment_id': hashlib.md5(f"gpw_sp_{attack_name}_{idx}".encode()).hexdigest()[:12],
                    'model_family': model_config['name'],
                    'watermark_method': 'GPW-SP',
    ...
    print("\n" + "=" * 70)
    print("GPW-SP EXPERIMENTS COMPLETE")
    print("=" * 70)
