#!/usr/bin/env python3
"""
Experiment: Synonym Attack on Watermarked Text

Tests watermark detection after applying synonym substitution attack.
This is the simplest attack - submit first.

Usage:
    python experiment_scripts/exp_attack_synonym.py --model opt-1.3b --watermark unigram --num_samples 200
"""
...
DTYPE = config.DTYPE
SYNONYM_ATTACK_CONFIG = config.SYNONYM_ATTACK_CONFIG
...
def main():
    parser = create_experiment_parser("Synonym Attack Experiment")
    parser.add_argument('--edit_rate', type=float, default=0.3,
                        help='Synonym substitution rate')
    args = parser.parse_args()
    
    print("=" * 80)
    print("EXPERIMENT: Synonym Attack on Watermarked Text")
    print("=" * 80)
    print(f"Model: {args.model}")
    print(f"Watermark: {args.watermark}")
    print(f"Num samples: {args.num_samples}")
    print(f"Edit rate: {args.edit_rate}")
    ...
    _, prompts = split_human_ai_data(c4_data, split_ratio=0.0)  # Use all for AI generation
    ...
    # Load attack
    print(f"\n[4/6] Loading attack: Synonym...")
    attack_params = {"edit_rate": args.edit_rate}
    attack = load_attack("synonym", attack_params)
    print(f"✓ Attack loaded: {attack.__class__.__name__}")
    
    # Setup CSV logger
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_csv = os.path.join(args.output_dir, 
                              f"exp_attack_synonym_{args.model}_{args.watermark}_{timestamp}.csv")
    ...
    print(f"\nOriginal Mean Z-score: {metrics['original_mean_z_score']:.2f}")
    print(f"Attacked Mean Z-score: {metrics['attacked_mean_z_score']:.2f}")
    print(f"Z-score Drop: {metrics['z_score_drop']:.2f}")
    print(f"\nResults saved to:")
    print(f"  - {output_csv}")
