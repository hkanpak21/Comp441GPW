#!/usr/bin/env python3
"""
Experiment: Copy-Paste (Mixing) Attack on Watermarked Text

Tests watermark detection after mixing watermarked and human text.
Medium complexity attack - submit fourth.

Usage:
    python experiment_scripts/exp_attack_copypaste.py --model opt-1.3b --watermark unigram --num_samples 200
"""
...
DTYPE = config.DTYPE
COPYPASTE_ATTACK_CONFIG = config.COPYPASTE_ATTACK_CONFIG
...
def main():
    parser = create_experiment_parser("Copy-Paste Attack Experiment")
    parser.add_argument('--mix_ratio', type=float, default=0.5,
                        help='Ratio of human text to mix in')
    args = parser.parse_args()
    
    print("=" * 80)
    print("EXPERIMENT: Copy-Paste (Mixing) Attack on Watermarked Text")
    print("=" * 80)
    ...
    c4_data = load_c4_dataset(num_samples=args.num_samples * 2, seed=args.seed)  # Need double for mixing
    human_texts, prompts = split_human_ai_data(c4_data, split_ratio=0.5)
    print(f"✓ Loaded {len(prompts)} prompts and {len(human_texts)} human texts")
    ...
    # Load attack
    print(f"\n[4/6] Loading attack: CopyPaste...")
    attack_params = {"n_segments": 3, "watermark_ratio": args.mix_ratio}
    attack = load_attack("copypaste", attack_params)
    print(f"✓ Attack loaded: {attack.__class__.__name__}")
    
    # Setup CSV logger
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_csv = os.path.join(args.output_dir, 
                              f"exp_attack_copypaste_{args.model}_{args.watermark}_{timestamp}.csv")
    
    fieldnames = [
        'sample_id', 'prompt', 'original_text', 'human_text', 'attacked_text',
    ...
    for idx, (prompt, human_text) in enumerate(zip(prompts, human_texts)):
        if (idx + 1) % 10 == 0:
        ...
        try:
            ...
            # Apply attack (mix with human text)
            attacked_text = attack(original_text, human_text=human_text)
